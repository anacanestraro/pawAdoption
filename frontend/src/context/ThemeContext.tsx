...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const applyTheme = (t: Theme) => {
    ...
    const bgUrl = `url("data:image/svg+xml;utf8,${svg}")`
    document.body.style.backgroundImage = bgUrl
    document.body.style.backgroundSize = '120px 120px'
    document.body.style.backgroundRepeat = 'repeat'

    // Aplica o mesmo padrão de patinhas do body no footer (light = azul, dark = branco)
    document.querySelectorAll('.appfooter, .appfooter-compact').forEach(el => {
      const htmlEl = el as HTMLElement
      htmlEl.style.backgroundImage = bgUrl
      htmlEl.style.backgroundSize = '120px 120px'
